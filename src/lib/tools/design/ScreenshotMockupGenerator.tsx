...
export default function ScreenshotMockupGenerator() {
  const [input, setInput] = useState('')
  const [output, setOutput] = useState('')
  const [error, setError] = useState('')
  
  function run() {
    try { setError(''); setOutput((input ? 'Design canvas ready (' + input.length + ' chars of source)' : 'Describe your design (or paste source) above')) }
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4">
      ...
      <Export fileName="ScreenshotMockupGenerator-output.txt" data={output} disabled={!output} />
    </div>
  )
}
