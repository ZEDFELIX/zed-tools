...
import { countWords, countChars, countLines, toCase, countSentences } from '../../../lib/engines/textEngine'
...
export default function CharacterCounter() {
  ...
  function run() {
    try { setError(''); setOutput(countWords(input) + ' words, ' + countChars(input) + ' chars, ' + countLines(input) + ' lines') }
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4">
      ...
      <Export fileName="CharacterCounter-output.txt" data={output} disabled={!output} />
    </div>
  )
}
