...
export default function MPesaQr() {
  const [input, setInput] = useState('')
  const [output, setOutput] = useState('')
  const [error, setError] = useState('')
  
  function run() {
    try { setError(''); setOutput((input ? 'QR payload ready: ' + input.length + ' chars' : 'No payload yet')) }
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4">
      ...
      <Export fileName="MPesaQr-output.txt" data={output} disabled={!output} />
    </div>
  )
}
