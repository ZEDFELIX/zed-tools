...
export default function DigitalBusinessCard() {
  ...
  function run() {
    try { setError(''); setOutput((input ? 'Document ready (' + input.length + ' fields)' : 'Enter document details above')) }
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4">
      ...
      <Export fileName="DigitalBusinessCard-output.txt" data={output} disabled={!output} />
    </div>
  )
}
