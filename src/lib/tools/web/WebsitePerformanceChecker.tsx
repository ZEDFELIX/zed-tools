...
export default function WebsitePerformanceChecker() {
  const [input, setInput] = useState('')
  const [output, setOutput] = useState('')
  const [error, setError] = useState('')
  
  function run() {
    try { setError(''); setOutput('Ready. Paste input and press Run.') }
  ...
  return (
    <div className="mx-auto w-full max-w-3xl space-y-4">
      ...
      <Export fileName="WebsitePerformanceChecker-output.txt" data={output} disabled={!output} />
    </div>
  )
}
